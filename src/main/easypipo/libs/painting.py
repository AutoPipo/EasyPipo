...
from ..libs.utils import is_cuda_cv
...
class Painting:
    ...
    def blurring(self, 
        ...
        blurring = None
        if is_cuda_cv():
            qimg_cuda = cv2.cuda_GpuMat()
            qimg_cuda.upload(qimg)
            blurring_cuda = cv2.cuda.bilateralFilter(qimg_cuda, radius, sigmaColor, 60)
            blurring = blurring_cuda.download()
        else:
...
def imageExpand(image, guessSize=False, size = 3):
    ...
    if is_cuda_cv():
        ...
        img_cuda = cv2.cuda.resize(img_cuda, None, fx=size, fy=size, interpolation=cv2.INTER_LINEAR)
        image = img_cuda.download()
